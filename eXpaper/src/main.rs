...
fn main() -> ! {
    ...
    let spi_device = ExclusiveDevice::new(spi_bus, cs, Delay::new())
    ...
    loop {
        ...
        // driver.full_update(&display).unwrap(); // 3.05 seconds frame time
        driver.fast_update(&display).unwrap(); // 1.92 seconds frame time
        let end = Instant::now();

        let diff = (end - start).as_micros() as f32 / 1_000_000f32;

        info!("Frame Time: {diff}s");
